<template>
  ...
  <q-page-sticky position="bottom-right" :offset="[38, 38]">
    <q-btn fab icon="add" color="primary" :to="{ name: 'company-create' }" />
  </q-page-sticky>
</template>
...
const rows = computed(() => $store.getters['adminCompanies/companies'] ?? []);
...
const storeMeta = computed(
  () => $store.getters['adminCompanies/pagination'] ?? {},
);
...
async function onRequest(props) {
  ...
  try {
    ...
    const meta = $store.getters['adminCompanies/pagination'] ?? {};
    paginationLocal.value.rowsNumber =
      meta.total ?? paginationLocal.value.rowsNumber;
...
const clickRow = (evt, row) => {
  $router.push({ name: 'company-edit', params: { id: row.id } });
};
